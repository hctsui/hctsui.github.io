# 網站與雙語履歷管理說明

日常入口：<https://hctsui.github.io/admin/>

## 核心資料結構

網站採用「頁面 → 類別 → 項目」三層結構：

- **頁面**決定導覽位置、網址、頁首與主色。
- **類別**就是頁面中的大區塊；類別的「顯示風格」決定其中項目的網站與 PDF 排版。
- **項目**必須屬於一個類別。類別移到另一頁時，其中項目會一起移動。

所有修改先保存在目前瀏覽器的草稿中；按「直接送出修改」後，Cloudflare Worker 會安全地代為建立 GitHub Issue，由自動流程成功套用後才會改動正式資料。

## 直接送出到 GitHub

- 第一次按「直接送出修改」會前往 GitHub 驗證帳號，成功後自動接續送出。
- 同一台裝置的登入保存 14 天；瀏覽器只保存網站專用的簽章憑證，不保存 GitHub Token。
- 批次資料直接送到 Cloudflare Worker，會在伺服器端壓縮並建立 Issue，不使用剪貼簿，也不必開啟 GitHub Issue 畫面。
- 重複點擊或網路回應中斷時會沿用同一送出識別碼，避免建立重複 Issue。
- Admin 會顯示處理進度；確認 `website-form-applied` 後才可清除該批草稿。若送出後又建立新草稿，系統不會誤刪。
- 「改用 GitHub Issue 手動送出」與「複製技術資料」永久保留作為備用。

## 項目清單

「項目」分頁的每筆資料會顯示：

- 項目類型。
- 所在頁面。
- 所在類別；頁面項目則顯示語言版本。

「一般內容」篩選會同時顯示標準時間軸、雙欄紀錄、標題清單、資訊卡片與標籤列表；榮譽與四種活動可分別篩選。

## 一般內容的六種顯示風格

一般內容新增與編輯時都可選擇顯示風格。風格只描述排版，不限制內容用途；儲存時必須選擇一個使用相同風格的類別。

| 顯示風格 | 排版概念 | 常見用途範例 |
| --- | --- | --- |
| 標準時間軸 | 日期在左，標題、機構與說明在右 | 一般經歷、服務紀錄 |
| 雙欄紀錄 | 日期欄與內容欄分明，內容採分層顯示 | 學歷、長期職務 |
| 標題清單 | 標題為主，補充文字在下一行 | 研究方向、主題清單 |
| 資訊卡片 | 每筆內容形成獨立卡片 | 聯絡方式、重點資訊 |
| 標籤列表 | 短標籤與內容並列 | 語言、技能、個人資料 |
| 精簡時間軸 | 年份與短內容壓縮在一行 | 榮譽、獎項、簡短里程碑 |

表單內有六種風格的即時縮圖；選擇風格後，符合的類別才會出現在「所屬類別」選單。右側預覽會明確顯示修改前後的風格與類別。編輯既有項目時也能改風格；若跨越榮譽與其他一般內容的資料區塊，Admin 會安全地建立成對的刪除／新增草稿，並由同一批次送出。風格變更自動產生的類別／排序草稿會與內容草稿連動，刪除草稿或改回原風格與類別時會一起還原。

## 統一排序與搬移

「排序」依頁面顯示所有類別與項目：

- 調整同一頁面中類別的順序。
- 調整類別內項目的順序。
- 在單一類別內依日期新到舊、日期舊到新或名稱快速排序。
- 每個項目都顯示「搬移」選單；選單列出所有能接受該項目格式的類別。
- 沒有其他相容類別時，「搬移」選單仍會顯示，但呈停用狀態。
- 「PDF 履歷」檢視只調整履歷類別順序，因此項目搬移選單停用。

「搬移」可以跨頁面；項目會跟著目標類別出現在新的頁面與 PDF 區段。



## 首頁內容

「項目」中的頁面清單會顯示「首頁」。編輯首頁時，表單只開放首頁最上方的五行文字與按鈕清單：

- 系所／學校第一行（中英文）。
- 英文姓名。
- 中文姓名。
- 身分文字（中英文）。
- 說明文字（中英文）。
- 首頁按鈕的中英文文字與 URL。

首頁按鈕可以新增或刪除，排列順序就是網站顯示順序。照片與說明在「網站設定」修改；精選論文與近期活動在「排序」修改；聯絡區由其內容項目管理。

首頁內容會以內部管理項目保存，因此沿用既有的草稿、預覽、衝突檢查、修改歷史與還原機制；該內部項目不會出現在一般項目清單或公開網站類別中。


## 首頁精選與近期活動

在「排序」選擇「首頁」後，可設定「精選論文」與「近期活動」的選取方式和顯示順序。

- 精選論文：最新 N 篇、最舊 N 篇，或手動選擇。
- 近期活動：最近 N 筆、日期最遠 N 筆，或手動選擇。
- 自動模式先依日期決定入選的 N 筆，再套用管理者調整的相對順序；新資料進入前 N 筆時，候選集合仍會自動更新。
- 手動模式可自行加入、移除及排序。
- 每個首頁區塊以小標籤顯示「目前無變更」或「有未送出變更」。
- 變更先保存為本機草稿；首頁草稿會列入「草稿」分頁，可修改、預覽或單獨移除。

## 頁面與類別

頁面與類別都從「新增」建立，也能在「項目」搜尋與編輯。新增頁面時先選「一般頁面」或「課程頁面」：一般頁面沿用頁首、類別與內容模板；課程頁面使用獨立的課程資訊、日期／主題／教材表格，網址位於 `teaching/<slug>/`。既有頁面都視為一般頁面。系統頁面「聯絡表單頁面」與「404 頁面」也會出現在一般頁面清單；它們不能刪除。404 的編輯入口會開啟「網站設定 → 一般設定 → 404 頁面設計」。

Teaching 項目的「課程資訊連結」可選站內課程頁面，選單只列課程頁面；若授課教師另有網站，也可改選「外部課程網址」並貼上 `http` 或 `https` 連結。兩者在前台都顯示為同一個「課程資訊」按鈕。

- 一般頁面可設為雙語、僅中文或僅英文；課程頁面只使用一種語言，建立時選英文或中文（預設英文）。建立後語言版本固定。
- 類別可編輯中英文小字、大標題、簡介、所在頁面，以及是否顯示於網站或 PDF。
- 類別顯示風格建立後固定；需要另一種排版時新增相應風格類別，再搬移項目。
- 刪除含項目的類別時，必須先把項目移到同格式的其他類別。

## Conference 與 Organization

學術會議與學術籌辦是兩種完全獨立的項目，不會自動複製或連動。需要同一活動同時出現在兩區時，建立兩筆資料。

## 中英文網站與 PDF 履歷

同一份 `content/site.json` 產生英文網站、中文網站、英文 PDF 履歷與中文 PDF 履歷。類別標題、項目所屬類別與排序會同步到 PDF；只有勾選「顯示於 PDF 履歷」的類別會輸出。

中文履歷標題及題目支援：

- `$t$` 或 `\(t\)`：數學模式。
- `[i]t[/i]` 或 `<em>t</em>`：斜體／數學變數。

## 學術訪問 Funding

學術訪問可填資助機構與資助計畫。開啟自動生成說明後，系統會分別產生對應語言的資助句子，且不會被一般的中英補填覆蓋。

## 資料庫：中英對照與人名連結

Admin 的「資料庫」分成兩個子類型，切換按鈕與上方主分頁之間保留獨立間距：

- **中英對照**：新增、修改、刪除、合併與排序詞條和標籤，也可匯入／匯出 JSON。大型字詞庫會自動 gzip 壓縮，避免 GitHub Issue 欄位長度限制。
- 右側「中英對照」預覽會分開列出文字、詞條標籤、標籤名稱與順序差異；修改前後逐欄比較，黃色「已修改」標記就是實際變動的位置。
- **人名連結**：集中管理英文姓名、中文姓名、其他拼法與學術網址。其他拼法每行一個，例如 `Tsui, Hung-Chun`。
- 編輯論文作者時，輸入框下方只會列出可能的人名；必須手動點選才會填入，不會因相似姓名自動替換。
- 人名精確比對會套用到整個公開網站，包括首頁說明、指導教授、活動、教學及論文作者。既有超連結、粗體、程式碼、數學標記與 BibTeX 不會被重複包覆。
- 人名資料保存於 `content/people.json`；沒有匹配或沒有網址時維持普通文字，PDF CV 不加入人名連結。
- 「人名一致性檢查」會掃描論文作者的中英文配對，找出與人名資料庫標準姓名不一致的項目。操作方式和「中英對照」一致，可逐筆套用、全部套用、忽略或清除忽略；修正仍先進入一般草稿。

## 論文引用與 arXiv 通知

- 公開網站的引用入口顯示為 **Cite**，並和 arXiv、PDF 使用完全相同的膠囊按鈕樣式；點擊後可在 **biblatex** 與 **LaTeX `ibitem`** 間切換，兩種格式都有獨立複製按鈕。
- 論文表單可分別手動覆寫 BibTeX 與 `\bibitem`；任一欄留白時，網站會依作者、題目、年份、arXiv／DOI 自動產生。
- `.github/workflows/check-arxiv.yml` 每週一上午 9:15（Asia/Tokyo）執行，也可從 Actions 手動執行。
- `tools/check_arxiv.py` 使用 arXiv 官方 API 搜尋 `Hung-Chun Tsui`，再以完整作者姓名做第二次精確比對，避免只因姓氏或相似姓名而產生通知。
- 新結果保存在 `content/arxiv-suggestions.json`，不會直接寫入公開論文列表。
- Admin 頁首的「通知」按鈕位於「開啟完整使用手冊」左側。每張通知右上角都有明確的「☆ 加星號／★ 已加星號」按鈕；星號、已讀與刪除狀態會和其他草稿一起送出。
- 按「加入新增草稿」只會建立一般論文草稿並開啟編輯表單；請確認中文題目、中文作者、PDF 與正式刊登資訊後，再和其他草稿一起送出。
- 按「忽略」只會先加入本機忽略草稿；送出 Batch Issue 後才會永久寫入忽略清單。忽略操作同樣支援衝突檢查與七日還原。
- 已存在於網站或已加入本機論文草稿的 arXiv ID 不會重複顯示。

## 網站設定

「網站設定」依序分成 **一般設定 → 聯絡表單 → SEO／OG → 流量統計**。四個區塊都沿用本機草稿、右側逐欄預覽、Batch Issue、衝突檢查與七日還原；網站設定草稿會和內容草稿一起顯示在主「草稿」分頁。

### 一般設定

進入一般設定後，先選擇與「新增活動」相同形式的功能按鈕：

- **封面照片編輯**：選擇首頁封面、備用圖片、雙語替代文字、說明文字及圖片對齊位置。圖片欄位會搜尋 `assets/images/` 的媒體索引並提供可點選候選。
- **導覽列調整**：啟用或關閉頂部搜尋框、設定中英文 placeholder、管理聯絡表單捷徑，並可直接前往頁面順序及「顯示於導覽列」設定。
- **頁尾設定**：新增中英文項目、超連結、圖標、開新分頁、對齊方式及順序。選擇自訂圖標時，建議放在 `assets/images/`，並可從媒體候選選取。
- **404 頁面設計**：設定雙語文字、按鈕、自動返回、導覽列、頁尾與配色。
- **網站識別**：設定導覽列品牌名稱及行動版選單文字。

頁面是否顯示在導覽列及其順序，在「頁面與類別／排序」管理；一般設定的「導覽列調整」管理搜尋功能與聯絡捷徑。

### 頂部搜尋

- 搜尋框會出現在所有公開頁面的頂部導覽列，包含 404。
- 搜尋資料由 `tools/build_search_index.py` 從目前的頁面、類別與項目生成至 `content/search-index.json`。
- 搜尋結果依目前語言顯示，並可直接前往對應頁面或項目錨點。
- 關閉搜尋後，所有頁面會同步隱藏搜尋框，不需要逐頁修改 HTML。

### 圖片與媒體檔案

- 公開圖片放在 `assets/images/`。
- OG 預設圖片、各頁 OG 圖片、封面照片及自訂圖標欄位會讀取 `content/media.json`，輸入檔名關鍵字即可選取 repository 內的圖片。
- 演講投影片統一放在 `files/slides/`；論文 PDF 統一放在 `files/papers/`。
- 連結欄位會依欄位用途與目前項目的標題、年份等資料，從媒體索引顯示相似檔名候選。點選候選只填入路徑，不會自動發布或覆蓋檔案。
- `tools/build_media_manifest.py` 每次建置會重新產生 `content/media.json`；新增檔案後不需要手動編輯索引。

### 聯絡表單

- 首頁不直接嵌入完整表單。聯絡區會顯示 **Contact Form／聯絡表單** 項目，連結文字為 **Fill out／填寫**。
- 點「Fill out／填寫」會開啟獨立的 `contact.html` 或 `zh/contact.html`。
- Admin 先顯示 **表單設定** 與 **頁面設計**。表單設定管理傳送方式、固定 Email 主旨與欄位文字；頁面設計管理雙語標題、導覽列、頁尾與配色。
- 可選 **Web3Forms Email** 或 **Cloudflare Worker 橋接**。固定通知主旨會套用到每一封信，訪客填寫的主旨只放在信件內容中。
- Email 模式只需要 Web3Forms Access Key；Worker 模式會另外建立不含個資的匿名 Admin 通知。GitHub Actions 與 Pages 發布可能造成短暫延遲。
- 完整姓名、Email、訪客主旨與訊息不會寫入公開 repository。
- 詳細步驟見 `integrations/CONTACT-FORM-SETUP.md`。

### SEO／OG

- 可設定基準網址、網站名稱、預設分享圖片，以及每頁中英文 SEO 標題、Meta description、OG 標題、OG 描述與個別分享圖片。
- 圖片欄位可直接搜尋並選擇 `assets/images/` 中的圖片。
- 建置時自動產生 canonical、Open Graph、Twitter Card、中文／英文 hreflang 與 x-default。
- `sitemap.xml` 只列出中英文首頁、履歷、論文、學術活動、教學與聯絡頁面；建置時自動更新。
- `robots.txt` 阻擋 Admin、審查資料、內容資料檔與管理工具。Admin、Admin 手冊、中英文審查資料與 404 另有 `noindex,nofollow`。
- Google Search Console 應建立 URL-prefix property `https://hctsui.github.io/`，驗證後送出 `https://hctsui.github.io/sitemap.xml`。

### 流量統計

- 可同時啟用 Cloudflare Web Analytics 與 Google Analytics 4、只啟用其中一家，或全部關閉；兩邊識別碼會保留。兩家同時啟用時，兩個後台都會各自計算同一批訪客，請勿把兩邊數字相加當成總流量。
- Cloudflare 使用 32 字元 Site Token；Google 使用 `G-...` Measurement ID。既有 Cloudflare Token 會自動沿用。
- 追蹤碼只加入公開頁面與 `404.html`，不加入 `/admin/`。
- Admin 可直接查看今天、7 天、30 天與 90 天的彙整報表，並保留開啟官方完整儀表板的按鈕。
- 報表沿用 GitHub 14 天管理員登入；Cloudflare API Token 與 Google 服務帳戶只存在 Worker Secret，不會保存到 Admin 或 repository。
- 第一次設定報表讀取權限請依照 `integrations/CONTACT-FORM-SETUP.md` 的「Admin 流量報表」段落。

### 網站設定草稿與預覽

- 草稿採正規化後的語意比較；欄位改回原值時會自動清除對應的本機草稿。沒有變更時不顯示多餘提示。
- 右側預覽會逐項列出變動欄位、修改前值與修改後值；主「草稿」分頁也會列出網站設定草稿。

## GitHub Issue 與自動流程

所有內容修改都從 Admin 送出。Admin 會建立批次 Issue，由自動流程套用資料、重建中英文網站與兩份 CV。

每日自動流程只重建日期相關內容；部署流程負責執行測試、重建網站、編譯兩份 PDF CV 並發布 GitHub Pages。

## 排序重設與草稿預覽

- 「排序」中每個類別卡都有「重設」。一般類別會還原該類別的位置、項目順序與搬移結果；首頁精選論文與近期活動則各自重設，不會互相影響。
- 首頁變更狀態依實際模式、N 值和顯示順序判斷。若操作後回到原始結果，狀態會自動恢復為「目前無變更」。
- 頁面、類別、項目排序與搬移會在「草稿」分頁顯示一筆「頁面／類別／排序」管理列，可回到排序修改、開啟詳細預覽或重設整組草稿。
- 右側預覽會逐欄列出頁面與類別的修改前後值，並用相同卡片樣式顯示首頁順序、一般內容風格、項目搬移及 PDF 履歷類別順序。預覽右上角有與其他操作一致的「關閉」按鈕，只關閉畫面而不影響草稿。
- 類別用途說明採小字提示，避免占用表單空間。


## URL 與電子郵件連結

一般網址使用 `http://` 或 `https://`；電子郵件項目的 URL 可填 `mailto:name@example.com`。Admin 會保留其他網址驗證，只額外允許格式正確的 `mailto:`。


### Batch 顯示 Worker URL 缺失

若 GitHub Action 顯示 `Worker URL is missing or invalid`，代表聯絡表單已勾選啟用且傳送模式是 Cloudflare Worker，但尚未填入完整 Worker URL。請回到 Admin → 網站設定 → 聯絡表單，選擇其中一種處理方式：

1. 填入完整的 `https://...workers.dev` 網址；
2. 改選 Web3Forms Email 並填入 Access Key；
3. 尚未設定完成時，先取消「啟用聯絡表單」。

修正設定後重新建立 Batch Issue；失敗的 Issue 不會自動套用新內容。
