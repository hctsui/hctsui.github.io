...
(function installAdminShellImmediately(){
  ...
  function installHead(){
    ...
    let touch=head.querySelector('link[data-admin-touch-icon]');
    if(!touch){touch=document.createElement('link');touch.rel='apple-touch-icon';touch.dataset.adminTouchIcon='';head.append(touch)}
    touch.href='admin-icon.png';
...
/* Homepage selections, general-content style editor, and Admin compatibility
   fixes. This file loads after the layout manager and intentionally owns the final
...
/* The generic URL field is also used for email links.  The legacy validator
   accepts only web URLs, so keep its other checks but allow a valid mailto:
   value without weakening validation for ordinary links. */
const homepageBaseValidateEditorObject=validateEditorObject;
validateEditorObject=function(type,o){
  const errors=homepageBaseValidateEditorObject(type,o),mailtoPaths=new Set();
  for(const [path,,kind] of (SCHEMA[type]||[])){
    if(kind!=='url')continue;
    const value=String(o?.[path]||'').trim();
    if(/^mailto:[^\s@]+@[^\s@]+$/i.test(value))mailtoPaths.add(path);
  }
  return errors.filter(message=>![...mailtoPaths].some(path=>message===`${path} 只允許 http 或 https`));
};
...
function generalStyleCards(selected){
  const current=selected?(GENERAL_STYLE_INFO[selected]?.name||GENERAL_FORMAT_LABELS[selected]||selected):'尚未選擇';
  return`<details class="general-style-guide" open><summary><span class="general-style-summary-row"><span class="general-style-summary-copy"><strong>選擇顯示風格</strong><small>點選下方卡片；可用左側箭頭收合。</small></span><span class="general-style-current" data-general-style-current>${esc(current)}</span></span></summary><div class="general-style-grid" role="radiogroup" aria-label="顯示風格">${GENERAL_FORMAT_ORDER.map(kind=>{const info=GENERAL_STYLE_INFO[kind],active=selected===kind;return`<button type="button" class="general-style-card ${active?'selected':''}" data-style-choice-kind="${kind}" aria-pressed="${active?'true':'false'}"><div class="general-style-card-head"><strong>${esc(info.name)}</strong><span class="general-style-check" aria-hidden="true">${active?'✓':''}</span></div><div class="general-style-sample ${kind}">${info.sample}</div><p>${esc(info.summary)}</p></button>`}).join('')}</div></details>`
}
function updateGeneralStyleCards(root,kind){
  root.querySelectorAll('[data-style-choice-kind]').forEach(card=>{const active=card.dataset.styleChoiceKind===kind;card.classList.toggle('selected',active);card.setAttribute('aria-pressed',active?'true':'false');const check=card.querySelector('.general-style-check');if(check)check.textContent=active?'✓':''});
  const current=root.querySelector('[data-general-style-current]');if(current)current.textContent=kind?(GENERAL_STYLE_INFO[kind]?.name||GENERAL_FORMAT_LABELS[kind]||kind):'尚未選擇';
}
...
function decorateGeneralEditor(type,record){
  ...
  const styleBlock=document.createElement('div');styleBlock.className='general-format-field';styleBlock.hidden=true;
  const editing=!!record,initial=editing?String(record.type||type):(type==='generic'?'':type);
  styleBlock.innerHTML=`<select id="generalContentFormat" aria-hidden="true" tabindex="-1"><option value="">請選擇顯示風格…</option>${GENERAL_FORMAT_ORDER.map(kind=>`<option value="${esc(kind)}" ${initial===kind?'selected':''}>${esc(GENERAL_FORMAT_LABELS[kind])}</option>`).join('')}</select>`;
  categoryBlock.before(styleBlock);styleBlock.insertAdjacentHTML('afterend',generalStyleCards(initial));
  const styleSelect=styleBlock.querySelector('#generalContentFormat'),categorySelect=categoryBlock.querySelector('#itemCategorySelector'),hint=categoryBlock.querySelector('.field-hint'),saveButton=root.querySelector('#saveEditor'),styleGuide=root.querySelector('.general-style-guide');
  const refresh=()=>{
  ...
  styleGuide?.addEventListener('click',event=>{const card=event.target.closest('[data-style-choice-kind]');if(!card)return;styleSelect.value=card.dataset.styleChoiceKind;refresh()});
...
function installActivityBackButton(root){
  ...
  const heading=[...root.children].find(node=>node.tagName==='H3');if(!heading)return;
  const header=document.createElement('div');header.className='page-editor-title activity-editor-title';
  const titleBox=document.createElement('div'),button=document.createElement('button');
  heading.replaceWith(header);titleBox.append(heading);
  button.type='button';button.className='button';button.setAttribute('data-back-to-activity-chooser','');button.textContent='← 返回活動類型';
  button.onclick=openAcademicEventChooser;
  header.append(titleBox,button);
}
...
function defaultHomeProfile(){return{
  ...
  description:{en:'Advisor: Professor Chieh-Yu Chang',zh:'指導教授：張介玉教授'},
  description_url:'https://sites.google.com/gapp.nthu.edu.tw/cychang/',
  description_link_text:{en:'Chieh-Yu Chang',zh:'張介玉教授'},
  actions:[
...
function normalizeHomeProfile(value){
  ...
  return{
    ...
    role:homeProfilePair(raw.role,defaults.role),
    description:homeProfilePair(raw.description||raw.advisor,defaults.description),
    description_url:homeProfileUrlValid(raw.description_url||raw.advisor_url)?String(raw.description_url||raw.advisor_url).trim():defaults.description_url,
    description_link_text:homeProfilePair(raw.description_link_text||raw.advisor_link_text,defaults.description_link_text),actions
  }
}
...
function homeProfilePageFormHtml(){
  ...
  <div class="pair-grid"><div class="field"><label>說明（英文）</label><input data-home-profile-field="description.en" value="${esc(p.description.en)}"></div><div class="field"><label>說明（中文）</label><input data-home-profile-field="description.zh" value="${esc(p.description.zh)}"></div></div>
...
function collectHomeProfile(root){
  ...
  return normalizeHomeProfile({...current,...value,kicker:{...current.kicker,...value.kicker},role:{...current.role,...value.role},description:{...current.description,...value.description},actions});
}
function validateHomeProfileForm(root){
  const raw=readNestedFields(root,'data-home-profile-field'),required=[raw.kicker?.en,raw.kicker?.zh,raw.name_en,raw.name_zh,raw.role?.en,raw.role?.zh,raw.description?.en,raw.description?.zh];
...
function saveHomeProfilePage(root){
  ...
  const profile=collectHomeProfile(root),formal=homeProfileRecord(site),after={
    ...
    title:{en:'Homepage profile',zh:'首頁內容'},description:{en:'',zh:''},profile
  };
...
function homeProfilePreviewHtml(before,after){
  ...
  const rows=[
    ...
    previewFieldRow('說明（英文）',oldProfile.description.en,newProfile.description.en),previewFieldRow('說明（中文）',oldProfile.description.zh,newProfile.description.zh),
    previewFieldRow('首頁按鈕',homeProfileButtonsPreview(oldProfile),homeProfileButtonsPreview(newProfile))
  ].filter(Boolean).join('');
  return`<details class="diff unified-preview" open><summary><strong>首頁內容</strong><span class="tag">頁面</span></summary><section class="unified-preview-card"><div class="unified-preview-card-head"><div><span class="preview-kicker">首頁</span><h4>首頁文字與按鈕</h4></div><span class="tag preview-status-changed">修改</span></div><div class="unified-preview-fields">${rows||'<p class="muted unified-preview-empty">沒有差異。</p>'}</div></section></details>`
}
