...
(function installSiteSettingsManager(){
  ...
  const SECTION_LABELS={general:'一般設定',contactForm:'聯絡表單',seo:'SEO／OG',analytics:'流量統計'};
  ...
  let remote=null,draft=null,ready=false,currentPage='home',currentSection='general',currentContactPanel='form',currentGeneralPanel='cover',siteDataCache={};
  ...
  const LEGACY_IMAGE_PATHS={'photo.jpg':'assets/images/photo.jpg','assets/photo-640.webp':'assets/images/photo-640.webp','assets/photo-960.webp':'assets/images/photo-960.webp','assets/photo-1440.webp':'assets/images/photo-1440.webp','assets/favicon.svg':'assets/images/favicon.svg'};
  function imagePath(value){const path=safeWebUrl(value,{relative:true});return LEGACY_IMAGE_PATHS[path]||path;}
  ...
  function defaultSettings(siteData){
    ...
    return {
      general:{schema_version:1,identity:{brand:{en:'HC Tsui',zh:'HC Tsui'},menu_label:{en:'Menu',zh:'選單'}},cover:{image:'assets/images/photo-960.webp',fallback:'assets/images/photo.jpg',alt:{en:'Landscape photograph',zh:'風景照片'},caption:{en:'Shikanoshima, Fukuoka · September 2025',zh:'日本福岡志賀島，2025 年 9 月'},object_position:'center center'},navigation:{search_enabled:true,search_placeholder:{en:'Search',zh:'搜尋'},search_label:{en:'Search this website',zh:'搜尋這個網站'},show_contact_shortcut:true}},
      ...
      seo:{schema_version:1,base_url:'https://hctsui.github.io',site_name:{en:'Hung-Chun Tsui',zh:'崔鴻竣'},default_image:'assets/images/photo-1440.webp',pages},
  ...
  function normalize(value,siteData){
    ...
    for(const id of ids){
      ...
      pages[id]={title:pair(raw.title,fallback.title,180),description:pair(raw.description,fallback.description,500),og_title:pair(raw.og_title,fallback.og_title,180),og_description:pair(raw.og_description,fallback.og_description,500),og_image:imagePath(raw.og_image)};
    }
    ...
    footerRows.forEach((raw,index)=>{
      ...
      items.push({id,text:pair(raw?.text,{en:'',zh:''},300),url:safeWebUrl(raw?.url,{relative:true,mailto:true}),icon,custom_icon:imagePath(raw?.custom_icon),alignment:ALIGNMENTS.some(x=>x[0]===raw?.alignment)?raw.alignment:'center',new_tab:Boolean(raw?.new_tab)});
    });
    const generalSource=source.general&&typeof source.general==='object'?source.general:{},generalIdentity=generalSource.identity&&typeof generalSource.identity==='object'?generalSource.identity:{},generalCover=generalSource.cover&&typeof generalSource.cover==='object'?generalSource.cover:{},generalNavigation=generalSource.navigation&&typeof generalSource.navigation==='object'?generalSource.navigation:{};
    const coverPosition=['center center','center top','center bottom','left center','right center'].includes(String(generalCover.object_position||''))?String(generalCover.object_position):defaults.general.cover.object_position;
    ...
    return {
      general:{schema_version:1,identity:{brand:pair(generalIdentity.brand,defaults.general.identity.brand,80),menu_label:pair(generalIdentity.menu_label,defaults.general.identity.menu_label,40)},cover:{image:imagePath(generalCover.image)||defaults.general.cover.image,fallback:imagePath(generalCover.fallback)||defaults.general.cover.fallback,alt:pair(generalCover.alt,defaults.general.cover.alt,160),caption:pair(generalCover.caption,defaults.general.cover.caption,240),object_position:coverPosition},navigation:{search_enabled:own(generalNavigation,'search_enabled')?Boolean(generalNavigation.search_enabled):true,search_placeholder:pair(generalNavigation.search_placeholder,defaults.general.navigation.search_placeholder,80),search_label:pair(generalNavigation.search_label,defaults.general.navigation.search_label,120),show_contact_shortcut:own(generalNavigation,'show_contact_shortcut')?Boolean(generalNavigation.show_contact_shortcut):true}},
      footer:{schema_version:2,items},
      seo:{schema_version:1,base_url:safeWebUrl(own(seoSource,'base_url')?seoSource.base_url:defaults.seo.base_url)||defaults.seo.base_url,site_name:pair(seoSource.site_name,defaults.seo.site_name,120),default_image:imagePath(own(seoSource,'default_image')?seoSource.default_image:defaults.seo.default_image)||defaults.seo.default_image,pages},
  ...
  function validate(){
    ...
    if(!value.general.identity.brand.en||!value.general.identity.brand.zh)errors.push('網站識別的中英文品牌名稱不可空白');
    for(const key of ['image','fallback']){const path=value.general.cover[key];if(path&&!safeWebUrl(path,{relative:true}))errors.push('封面照片必須是相對路徑或 http／https 網址');if(path&&!/^https?:\/\//i.test(path)&&!path.startsWith('assets/images/'))errors.push('網站內封面照片必須放在 assets/images/');}
    ...
    value.footer.items.forEach((item,index)=>{
      ...
      if(item.icon==='other'&&!item.custom_icon)errors.push(`頁尾第 ${index+1} 項選擇「其他」時必須填圖標檔案路徑`);if(item.custom_icon&&!/^https?:\/\//i.test(item.custom_icon)&&!item.custom_icon.startsWith('assets/images/'))errors.push(`頁尾第 ${index+1} 項的網站內圖標必須放在 assets/images/`);
    });
  ...
  function changeSections(before,after){
    const sections=[];
    const general=[];
    for(const [path,label] of [['identity.brand.en','英文品牌名稱'],['identity.brand.zh','中文品牌名稱'],['identity.menu_label.en','英文選單文字'],['identity.menu_label.zh','中文選單文字'],['cover.image','封面照片'],['cover.fallback','封面備援照片'],['cover.alt.en','封面英文替代文字'],['cover.alt.zh','封面中文替代文字'],['cover.caption.en','封面英文說明'],['cover.caption.zh','封面中文說明'],['cover.object_position','封面裁切位置'],['navigation.search_enabled','啟用導覽搜尋'],['navigation.search_placeholder.en','英文搜尋提示'],['navigation.search_placeholder.zh','中文搜尋提示'],['navigation.show_contact_shortcut','顯示聯絡表單捷徑']]){const a=valueAt(before.general,path),b=valueAt(after.general,path);if(!equal(a,b))general.push(changeItem(label,a,b));}
    if(general.length)sections.push(['一般設定',general]);
    const footer=[];
  ...
  function renderErrorPage(root=document.querySelector('#generalErrorPane')){if(!root)return;const e=draft.error_page;
  ...
  function adminPreviewUrl(value){const raw=String(value||'').trim();return /^https?:\/\//i.test(raw)?raw:'../'+raw.replace(/^\/+/, '');}
  function generalPanelButtons(){
    const buttons=[['cover','封面照片編輯'],['navigation','導覽列調整'],['footer','頁尾設定'],['error','404 頁面設計'],['identity','網站識別']];
    return `<div class="settings-intro"><strong>一般設定</strong><span>集中管理封面、導覽列、頁尾、404 與網站名稱。404 使用和其他頁面相同的公開導覽列。</span></div><div class="general-settings-choices">${buttons.map(([id,label])=>`<button class="general-settings-choice ${currentGeneralPanel===id?'active':''}" type="button" data-general-panel="${id}"><strong>${label}</strong></button>`).join('')}</div>`;
  }
  function renderCoverSettings(){const c=draft.general.cover;return `<div class="site-settings-card"><h3>首頁封面照片</h3><div class="cover-admin-preview"><img src="${esc(adminPreviewUrl(c.image))}" alt=""><span>${esc(c.caption.zh||c.caption.en)}</span></div><div class="field"><label>主要圖片</label><input data-general-field="cover.image" data-media-kind="images" value="${esc(c.image)}" placeholder="assets/images/photo-960.webp"><p class="field-hint">網站內圖片固定放在 assets/images/；輸入檔名會出現可點選建議。</p></div><div class="field"><label>備援圖片</label><input data-general-field="cover.fallback" data-media-kind="images" value="${esc(c.fallback)}" placeholder="assets/images/photo.jpg"></div><div class="pair-grid"><div class="field"><label>替代文字（英文）</label><input data-general-field="cover.alt.en" value="${esc(c.alt.en)}"></div><div class="field"><label>替代文字（中文）</label><input data-general-field="cover.alt.zh" value="${esc(c.alt.zh)}"></div></div><div class="pair-grid"><div class="field"><label>照片說明（英文）</label><input data-general-field="cover.caption.en" value="${esc(c.caption.en)}"></div><div class="field"><label>照片說明（中文）</label><input data-general-field="cover.caption.zh" value="${esc(c.caption.zh)}"></div></div><div class="field"><label>裁切位置</label><select data-general-field="cover.object_position">${[['center center','置中'],['center top','靠上'],['center bottom','靠下'],['left center','靠左'],['right center','靠右']].map(([v,l])=>`<option value="${v}" ${c.object_position===v?'selected':''}>${l}</option>`).join('')}</select></div></div>`;}
  function renderNavigationSettings(){const n=draft.general.navigation,pages=currentNavigationPages();return `<div class="site-settings-card"><h3>公開導覽列</h3><div class="form-options"><label class="switch"><input type="checkbox" data-general-field="navigation.search_enabled" ${n.search_enabled?'checked':''}>顯示搜尋框</label><label class="switch"><input type="checkbox" data-general-field="navigation.show_contact_shortcut" ${n.show_contact_shortcut?'checked':''}>顯示聯絡表單捷徑</label></div><div class="pair-grid"><div class="field"><label>搜尋提示（英文）</label><input data-general-field="navigation.search_placeholder.en" value="${esc(n.search_placeholder.en)}"></div><div class="field"><label>搜尋提示（中文）</label><input data-general-field="navigation.search_placeholder.zh" value="${esc(n.search_placeholder.zh)}"></div></div><div class="site-settings-subnote"><strong>目前顯示順序</strong><span>${pages.map(page=>esc(page.name?.zh||page.name?.en||page.id)).join('　')||'沒有導覽頁面'}</span></div><button class="button" type="button" data-open-page-manager>前往頁面與導覽排序</button></div>`;}
  function renderIdentitySettings(){const i=draft.general.identity;return `<div class="site-settings-card"><h3>網站識別</h3><div class="pair-grid"><div class="field"><label>品牌名稱（英文）</label><input data-general-field="identity.brand.en" value="${esc(i.brand.en)}"></div><div class="field"><label>品牌名稱（中文）</label><input data-general-field="identity.brand.zh" value="${esc(i.brand.zh)}"></div></div><div class="pair-grid"><div class="field"><label>選單按鈕（英文）</label><input data-general-field="identity.menu_label.en" value="${esc(i.menu_label.en)}"></div><div class="field"><label>選單按鈕（中文）</label><input data-general-field="identity.menu_label.zh" value="${esc(i.menu_label.zh)}"></div></div></div>`;}
  function renderGeneral(){const root=document.querySelector('#generalSettingsPane');if(!root)return;let body='';if(currentGeneralPanel==='cover')body=renderCoverSettings();else if(currentGeneralPanel==='navigation')body=renderNavigationSettings();else if(currentGeneralPanel==='identity')body=renderIdentitySettings();else if(currentGeneralPanel==='footer')body='<div id="generalFooterPane"></div>';else body='<div id="generalErrorPane"></div>';root.innerHTML=generalPanelButtons()+body;if(currentGeneralPanel==='footer')renderFooter(root.querySelector('#generalFooterPane'));if(currentGeneralPanel==='error')renderErrorPage(root.querySelector('#generalErrorPane'));}
  ...
  function render(){
    ...
    const panes={general:'generalSettingsPane',contactForm:'contactFormSettingsPane',seo:'seoSettingsPane',analytics:'analyticsSettingsPane'};
    Object.entries(panes).forEach(([key,id])=>{const pane=document.querySelector(`#${id}`);if(pane)pane.hidden=currentSection!==key;});
    renderGeneral();renderContactForm();renderSeo();renderAnalytics();renderStatus();
  }
  ...
  function installPanel(){
    ...
    panel?.addEventListener('click',event=>{
      ...
      const generalPanel=event.target.closest('[data-general-panel]');if(generalPanel){currentGeneralPanel=generalPanel.dataset.generalPanel;render();return;}
      if(event.target.closest('[data-open-page-manager]')){document.querySelector('[data-tab="content"]')?.click();setTimeout(()=>document.querySelector('[data-layout-section="pages"]')?.click(),0);return;}
      ...
      if(event.target.id==='resetSiteSettings'){if(confirm('放棄尚未送出的一般設定、聯絡表單、SEO／OG 與流量統計修改？')){draft=copy(remote);save();}return;}
    ...
    panel?.addEventListener('input',event=>{
      ...
      if(target.matches('[data-general-field]')){const value=target.type==='checkbox'?target.checked:target.value;updatePath(draft.general,target.dataset.generalField,value);save(false);return;}
  ...
  function installStyles(){
    const style=document.createElement('style');style.id='site-settings-admin-styles';style.textContent=`
      ...
      .general-settings-choices{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin:14px 0 18px}.general-settings-choice{min-height:72px;border:1px solid #d7c9bf;border-radius:14px;background:#fff;color:#2d2926;padding:16px;font:inherit;text-align:center;cursor:pointer}.general-settings-choice:hover,.general-settings-choice.active{border-color:#8d493d;background:#8d493d;color:#fff}.cover-admin-preview{display:grid;gap:8px;margin-bottom:14px}.cover-admin-preview img{width:min(100%,520px);max-height:260px;object-fit:cover;border-radius:12px;border:1px solid #ded3ca}.cover-admin-preview span{color:#6c625c;font-size:.78rem}.site-settings-subnote{display:grid;gap:5px;margin:12px 0;padding:11px;border-radius:10px;background:#f5eee8}.site-settings-subnote span{color:#6c625c}
      ...
      @media(max-width:700px){.general-settings-choices,.footer-preview-zones,.color-grid,.contact-settings-choices,.contact-preview-fields{grid-template-columns:1fr}.footer-preview-zones>div{justify-content:flex-start!important}.settings-diff-section li{grid-template-columns:1fr}.settings-arrow{display:none}.footer-editor-head{align-items:flex-start;flex-direction:column}}
    `;document.head.append(style);
  }
  window.siteSettingsDirty=dirty;window.siteSettingsOperation=operation;window.siteSettingsPreviewHtml=previewHtml;window.siteSettingsHistoryPreviewHtml=history=>previewHtml({before:history.before,after:history.after});window.validateSiteSettingsDraft=validate;window.clearSiteSettingsDraft=clearDraft;window.renderSiteSettings=render;window.openSiteSettingsSection=function(section,subsection){if(section==='footer'){section='general';subsection='footer';}if(section==='errorPage'){section='general';subsection='error';}currentSection=SECTION_LABELS[section]?section:'general';if(section==='contactForm')currentContactPanel=subsection==='design'?'design':'form';if(section==='general'&&['cover','navigation','footer','error','identity'].includes(subsection))currentGeneralPanel=subsection;render();};
  installStyles();installPanel();
  fetch('../content/site.json',{cache:'no-store'}).then(response=>response.json()).then(siteData=>{
    siteDataCache=siteData;remote=normalize({general:siteData.settings?.general,footer:siteData.settings?.footer,seo:siteData.settings?.seo,analytics:siteData.settings?.analytics,contact_form:siteData.settings?.contact_form,error_page:siteData.settings?.error_page},siteData);draft=loadSaved(remote);ready=true;render();if(typeof site!=='undefined'&&site&&typeof renderPreview==='function')renderPreview(false);
