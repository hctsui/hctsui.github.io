...
(function installSiteSettingsManager(){
  ...
  const SECTION_LABELS={footer:'頁尾',seo:'SEO／OG',analytics:'流量統計',errorPage:'404 頁面'};
  ...
  function render(){
    ...
    const panes={footer:'footerSettingsPane',seo:'seoSettingsPane',analytics:'analyticsSettingsPane',errorPage:'errorPageSettingsPane'};
    Object.entries(panes).forEach(([key,id])=>{const pane=document.querySelector(`#${id}`);if(pane)pane.hidden=currentSection!==key;});
    renderFooter();renderSeo();renderAnalytics();renderErrorPage();renderStatus();
  }
  ...
  function installPanel(){
    ...
    panel?.addEventListener('click',event=>{
      ...
      if(event.target.id==='resetSiteSettings'){if(confirm('放棄尚未送出的頁尾、SEO／OG、流量統計與 404 頁面修改？')){draft=copy(remote);save();}return;}
  ...
  window.siteSettingsDirty=dirty;window.siteSettingsOperation=operation;window.siteSettingsPreviewHtml=previewHtml;window.siteSettingsHistoryPreviewHtml=history=>previewHtml({before:history.before,after:history.after});window.validateSiteSettingsDraft=validate;window.clearSiteSettingsDraft=clearDraft;window.renderSiteSettings=render;
  installStyles();installPanel();
  fetch('../content/site.json',{cache:'no-store'}).then(response=>response.json()).then(siteData=>{
    siteDataCache=siteData;remote=normalize({footer:siteData.settings?.footer,seo:siteData.settings?.seo,analytics:siteData.settings?.analytics,error_page:siteData.settings?.error_page},siteData);draft=loadSaved(remote);ready=true;render();if(typeof site!=='undefined'&&site&&typeof renderPreview==='function')renderPreview(false);
