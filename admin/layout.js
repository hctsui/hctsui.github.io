...
const COURSE_SECTION_DETAIL_PREFIX='__course_section__:';
function coursePageData(value){
  ...
  const legacySections=(Array.isArray(source.sections)?source.sections:[]).filter(row=>row&&typeof row==='object');
  const encodedIds=new Set(details.filter(row=>row.id.startsWith(COURSE_SECTION_DETAIL_PREFIX)).map(row=>row.id.slice(COURSE_SECTION_DETAIL_PREFIX.length)));
  legacySections.forEach((row,index)=>{const id=String(row.id||`section-${index+1}`);if(!encodedIds.has(id))details.push({id:COURSE_SECTION_DETAIL_PREFIX+id,label:layoutPair(row.title),value:layoutPair(row.content)})});
...
function courseSectionRows(value){
  return coursePageData(value).details.filter(row=>row.id.startsWith(COURSE_SECTION_DETAIL_PREFIX)).map((row,index)=>({id:row.id.slice(COURSE_SECTION_DETAIL_PREFIX.length)||`section-${index+1}`,title:layoutPair(row.label),content:layoutPair(row.value)}));
}
function courseInfoDetailRows(value){return coursePageData(value).details.filter(row=>!row.id.startsWith(COURSE_SECTION_DETAIL_PREFIX))}
...
const COURSE_SECTION_PRESETS={announcements:{en:'Announcements',zh:'公告'},rules:{en:'Rules',zh:'規則'},homework:{en:'Homework',zh:'作業'},references:{en:'References',zh:'參考資料'}};
...
function courseSectionRowHtml(row={}){
  return `<div class="course-editor-row course-section-row" data-course-section-row data-row-id="${esc(row.id||courseEditorId('section'))}"><div class="course-editor-row-head"><strong>自由區塊</strong><div class="actions"><button class="button" type="button" data-move-course-row="-1">↑</button><button class="button" type="button" data-move-course-row="1">↓</button><button class="button danger" type="button" data-remove-course-row>移除</button></div></div><div class="pair-grid"><div class="field" data-page-language="en"><label>Title</label><input data-section-title="en" placeholder="Announcements, Policies, Resources…" value="${esc(row.title?.en||'')}"></div><div class="field" data-page-language="zh"><label>標題</label><input data-section-title="zh" placeholder="公告、規則、參考資料…" value="${esc(row.title?.zh||'')}"></div></div><div class="pair-grid"><div class="field" data-page-language="en"><label>Content</label><textarea data-section-content="en" placeholder="Write any course information here…">${esc(row.content?.en||'')}</textarea></div><div class="field" data-page-language="zh"><label>內容</label><textarea data-section-content="zh" placeholder="在這裡自由輸入內容…">${esc(row.content?.zh||'')}</textarea></div></div></div>`;
}
function courseMaterialRowHtml(material={}){
  return `<div class="course-material-row" data-course-material-row data-row-id="${esc(material.id||courseEditorId('material'))}"><div class="pair-grid"><div class="field" data-page-language="en"><label>教材名稱（英文）</label><input data-material-label="en" value="${esc(material.label?.en||'')}"></div><div class="field" data-page-language="zh"><label>教材名稱（中文）</label><input data-material-label="zh" value="${esc(material.label?.zh||'')}"></div></div><div class="course-material-url"><div class="field"><label>教材連結</label><input data-material-url data-path="pdf_url" type="url" placeholder="https://…" value="${esc(material.url||'')}"><p class="field-hint">操作方式與新增作品的「PDF 連結」相同：可直接貼外部網址，也可從出現的 R2 檔案建議中選擇。</p></div><button class="button danger" type="button" data-remove-course-row>移除教材</button></div></div>`;
}
...
function coursePageFieldsHtml(page){
  const course=coursePageData(page?.course),sections=courseSectionRows(page?.course),details=courseInfoDetailRows(page?.course);
  const presetButtons=Object.entries(COURSE_SECTION_PRESETS).map(([key,label])=>`<button class="button" type="button" data-add-course-section-preset="${key}">＋ ${esc(label.en)}</button>`).join('');
  return `<section class="course-editor-section"><div class="course-editor-section-head"><div><h3>課程資訊</h3><p class="field-hint">可自由新增機構、學期、授課教師、上課地點、Office hours 等欄位。</p></div><button class="button" type="button" data-add-course-detail>＋ 新增資訊欄位</button></div><div data-course-details>${details.map(courseDetailRowHtml).join('')}</div></section><section class="course-editor-section"><div class="course-editor-section-head"><div><h3>自由內容區塊</h3><p class="field-hint">標題與內容完全自由；下方預設按鈕只會幫你快速建立常用標題，建立後仍可自行改名。</p></div><button class="button" type="button" data-add-course-section>＋ 自訂區塊</button></div><div class="course-section-presets"><span>快速新增：</span>${presetButtons}</div><div data-course-sections>${sections.map(courseSectionRowHtml).join('')}</div></section><section class="course-editor-section"><div class="course-editor-section-head"><div><h3>日期、主題與教材</h3><p class="field-hint">每個日期可附多個講義、作業或解答；教材可直接貼外部連結，也可從現有 R2 檔案挑選。</p></div><button class="button" type="button" data-add-course-meeting>＋ 新增一列</button></div><div data-course-schedule>${course.schedule.map(courseMeetingRowHtml).join('')}</div></section><div class="pair-grid"><div class="field" data-page-language="en"><label>頁尾提醒（英文，可留白）</label><textarea data-course-footer="en">${esc(course.footer_note.en)}</textarea></div><div class="field" data-page-language="zh"><label>頁尾提醒（中文，可留白）</label><textarea data-course-footer="zh">${esc(course.footer_note.zh)}</textarea></div></div>`;
}
function collectCoursePage(root){
  ...
  const sections=[...root.querySelectorAll('[data-course-section-row]')].map(row=>({id:row.dataset.rowId,title:{en:row.querySelector('[data-section-title="en"]')?.value||'',zh:row.querySelector('[data-section-title="zh"]')?.value||''},content:{en:row.querySelector('[data-section-content="en"]')?.value||'',zh:row.querySelector('[data-section-content="zh"]')?.value||''}})).filter(row=>Object.values(row.title).some(Boolean)||Object.values(row.content).some(Boolean));
  ...
  const sectionDetails=sections.map(row=>({id:COURSE_SECTION_DETAIL_PREFIX+row.id,label:layoutPair(row.title),value:layoutPair(row.content)}));
  return{details:[...details,...sectionDetails],schedule,footer_note:{en:root.querySelector('[data-course-footer="en"]')?.value||'',zh:root.querySelector('[data-course-footer="zh"]')?.value||''}};
}
function bindCoursePageEditor(root){
  root.addEventListener('click',event=>{const button=event.target.closest('button');if(!button)return;if(button.hasAttribute('data-add-course-detail')){root.querySelector('[data-course-details]').insertAdjacentHTML('beforeend',courseDetailRowHtml());updatePageLanguageFields(root)}else if(button.hasAttribute('data-add-course-section')){root.querySelector('[data-course-sections]').insertAdjacentHTML('beforeend',courseSectionRowHtml());updatePageLanguageFields(root)}else if(button.hasAttribute('data-add-course-section-preset')){const preset=COURSE_SECTION_PRESETS[button.dataset.addCourseSectionPreset]||{en:'',zh:''};root.querySelector('[data-course-sections]').insertAdjacentHTML('beforeend',courseSectionRowHtml({title:preset,content:{en:'',zh:''}}));updatePageLanguageFields(root)}else if(button.hasAttribute('data-add-course-meeting')){root.querySelector('[data-course-schedule]').insertAdjacentHTML('beforeend',courseMeetingRowHtml());updatePageLanguageFields(root)}else if(button.hasAttribute('data-add-course-material')){button.closest('[data-course-meeting-row]').querySelector('[data-course-materials]').insertAdjacentHTML('beforeend',courseMaterialRowHtml());updatePageLanguageFields(root)}else if(button.hasAttribute('data-remove-course-row'))button.closest('[data-course-material-row],[data-course-detail-row],[data-course-section-row],[data-course-meeting-row]')?.remove();else if(button.hasAttribute('data-move-course-row')){const row=button.closest('[data-course-meeting-row],[data-course-section-row]'),delta=Number(button.dataset.moveCourseRow),sibling=delta<0?row?.previousElementSibling:row?.nextElementSibling;if(row&&sibling)row.parentElement.insertBefore(delta<0?row:sibling,delta<0?sibling:row)}});
}
...
function openPageTypeChooser(){
  ...
  box.innerHTML=`<div class="activity-chooser-head"><div><span class="eyebrow">新增頁面</span><h3>選擇頁面類型</h3></div></div><p class="field-hint">一般頁面使用現有的頁首、類別與內容排版；課程頁面使用課程資訊、自由內容區塊、日期、主題與教材表格。</p><div class="academic-event-choices page-type-choices"><button class="button academic-event-choice" type="button" data-page-type-choice="general"><strong>一般頁面</strong><span>現有模板；適合研究、活動或其他自訂內容</span></button><button class="button academic-event-choice" type="button" data-page-type-choice="course"><strong>課程頁面</strong><span>課程資訊、自由區塊、進度表與教材連結</span></button></div>`;
