...
function cardHtml(item){
  const star=`<button class="notification-icon-button ${item.starred?'starred':''}" type="button" title="${item.starred?'取消星號':'加星號'}" ${item.type==='arxiv'?`data-arxiv-star="${esc(item._arxiv.arxiv_id)}"`:`data-general-star="${esc(item.id)}"`}>${item.starred?'★':'☆'}</button>`;
  const read=`<button class="notification-icon-button" type="button" title="${item.read?'標記未讀':'標記已讀'}" ${item.type==='arxiv'?`data-arxiv-read="${esc(item._arxiv.arxiv_id)}" data-next="${item.read?'0':'1'}"`:`data-general-read="${esc(item.id)}" data-next="${item.read?'0':'1'}"`}>${item.read?'●':'○'}</button>`;
  let actions='';
