...
(function installR2StorageManager(){
  ...
  function preferredFolder(input){
    ...
    if(/lecture[_-]?notes?_url|notes_url/.test(raw))return'lecturenotes';
    if(/slides?_url|presentation_url/.test(raw))return'slides';
    if(/pdf_url|paper_url|manuscript_url/.test(raw))return'papers';
    if(/image|photo|icon|og_?image|thumbnail/.test(raw))return'images';
    const match=raw.match(/([a-z][a-z0-9_-]*)_url/);
    return match?safePrefix(match[1].replace(/_/g,'')):' ';
  }

  function score(item,query,folder){
    const hay=normalize(`${item.name} ${item.key} ${item.folder} ${item.url||''}`),needle=normalize(query);
    let value=item.folder===folder?120:0;
    if(!needle)return value;
    if(hay===needle)value+=350;
    else if(hay.startsWith(needle))value+=240;
    else if(hay.includes(needle))value+=160;
    for(const token of needle.split(/\s+/).filter(Boolean))if(hay.includes(token))value+=18;
    return value;
  }
  ...
  async function showSuggestions(input){
    ...
    try{
      const library=await loadLibrary(),query=input.value;
      const ranked=library.items.map(item=>({item,score:score(item,query,folder)}))
        .filter(row=>!query||row.score>0)
        .sort((a,b)=>b.score-a.score||a.item.name.localeCompare(b.item.name)).slice(0,10);
      popup.innerHTML=`<div class="r2-media-suggestion-title"><span>R2 儲存桶</span><button type="button" data-r2-open-settings>管理／上傳</button></div>${ranked.map(({item})=>`<button type="button" class="r2-media-choice" data-r2-media-url="${esc(item.url)}"><strong>${esc(item.name)}</strong><span>${esc(item.folder)} · ${humanSize(item.size)}</span></button>`).join('')||'<p>沒有符合的檔案</p>'}`;
      popup.hidden=false;
    }catch(reason){
      popup.innerHTML=`<div class="r2-media-suggestion-title"><span>${esc(reason.message)}</span><button type="button" data-r2-open-settings>開啟 R2 儲存桶</button></div>`;
      popup.hidden=false;
    }
  }

  function settingsPaneHtml(){
    return `<div class="settings-intro"><strong>R2 儲存桶</strong><span>直接管理 Cloudflare R2 的檔案。資料夾依物件 key 自動辨識，不限於 papers、slides、images；輸入新名稱即可使用 lecturenotes 或其他分類。</span></div>
  ...
  function installSettingsPane(){
    const panel=document.querySelector('#siteSettingsTab'),tabs=panel?.querySelector('.site-settings-tabs');
    if(!panel||!tabs)return false;
    if(!tabs.querySelector('[data-r2-settings-section]'))tabs.insertAdjacentHTML('beforeend','<button class="button" type="button" data-r2-settings-section>R2 儲存桶</button>');
    if(!panel.querySelector('#r2SettingsPane')){
      const analytics=panel.querySelector('#analyticsSettingsPane');
      analytics?.insertAdjacentHTML('afterend','<div id="r2SettingsPane" hidden></div>');
      panel.querySelector('#r2SettingsPane').innerHTML=settingsPaneHtml();
    }
    return true;
  }

  function setR2VisualState(){
    const panel=document.querySelector('#siteSettingsTab'),pane=panel?.querySelector('#r2SettingsPane');
    if(!panel||!pane)return;
    panel.querySelectorAll('[data-site-settings-section]').forEach(button=>button.classList.remove('active'));
    panel.querySelector('[data-r2-settings-section]')?.classList.toggle('active',r2Active);
  ...
  async function activateR2(input=null,force=false){
    currentInput=input;
    document.querySelector('[data-tab="siteSettings"]')?.click();
    installSettingsPane();
    r2Active=true;
    setR2VisualState();
  ...
  function deactivateR2(){
    r2Active=false;
    currentInput=null;
    const pane=document.querySelector('#r2SettingsPane');if(pane)pane.hidden=true;
    document.querySelector('[data-r2-settings-section]')?.classList.remove('active');
  }

  function setStatus(message,kind=''){
    const node=document.querySelector('#r2SettingsPane [data-r2-status]');
    if(!node)return;
    node.hidden=!message;
  ...
  async function renderLibrary(force=false){
    ...
    const pane=document.querySelector('#r2SettingsPane'),library=await loadLibrary(force),filter=pane.querySelector('[data-r2-folder-filter]');
  ...
  function useUrl(url,input=currentInput){
    if(!input||!url)return;
    input.value=url;
  ...
  async function uploadGeneral(){
    const pane=document.querySelector('#r2SettingsPane'),prefix=safePrefix(pane.querySelector('[data-r2-upload-folder]').value),files=[...pane.querySelector('[data-r2-upload-files]').files];
  ...
  async function applyHomePhotoSettings(urls){
    ...
    deactivateR2();
    document.querySelector('[data-site-settings-section="general"]')?.classList.add('active');
    if(typeof window.flash==='function')window.flash('封面照片已上傳，並建立網站設定草稿');
  }

  async function uploadHomePhoto(){
    const pane=document.querySelector('#r2SettingsPane'),file=pane.querySelector('[data-r2-home-photo]').files[0];
  ...
  document.addEventListener('focusin',event=>{const input=event.target.closest('input');if(input)showSuggestions(input);});
  document.addEventListener('input',event=>{
    const input=event.target.closest('input');if(input&&!input.closest('#r2SettingsPane'))showSuggestions(input);
  ...
  document.addEventListener('click',event=>{
    ...
    if(event.target.closest('[data-r2-settings-section]')){activateR2(null,true);return;}
    const choice=event.target.closest('[data-r2-media-url]');
    if(choice){useUrl(choice.dataset.r2MediaUrl,choice.closest('.r2-media-suggestion-popup')?.previousElementSibling);choice.closest('.r2-media-suggestion-popup').hidden=true;return;}
    if(event.target.closest('[data-r2-open-settings]')){const input=event.target.closest('.r2-media-suggestion-popup')?.previousElementSibling||null;activateR2(input,true);return;}
    ...
    const use=event.target.closest('[data-r2-use]');if(use){useUrl(use.dataset.r2Use);return;}
    ...
    document.querySelectorAll('.r2-media-suggestion-popup').forEach(popup=>{if(!popup.contains(event.target)&&popup.previousElementSibling!==event.target)popup.hidden=true;});
  });
