...
class ArxivBatchTests(unittest.TestCase):
    def test_ignore_operation_and_undo(self) -> None:
        ...
        arxiv_store = normalized_store({
            **empty_store(),
            "suggestions": [{
                "arxiv_id": "2608.01234", "title": "A New Function Field Result",
                "authors": ["Hung-Chun Tsui"], "published": "2026-08-03T00:00:00Z",
            }],
        })
        ...
        action, entry_id = apply_special(
            data, translations, history,
            {"op": "arxiv_suggestions", "before": copy.deepcopy(arxiv_store), "after": after},
            "issue-1-op-1", 1, datetime.now(timezone.utc), "digest-1",
            people=people, arxiv_store=arxiv_store,
        )
        ...
        undo_action, undo_id = apply_undo(
            ...
            "issue-2-op-1", 2, datetime.now(timezone.utc), "digest-2",
            people=people, arxiv_store=arxiv_store,
        )
...
class StaticArxivUiTests(unittest.TestCase):
    def test_notification_button_is_inserted_before_guide(self) -> None:
        ...
        for marker in (
            "data-notification-button", "actions.insertBefore(button,guide)", "加入新增草稿",
            "忽略", "arxivSuggestionsOperation", "arXiv metadata 來自 arXiv",
        ):
