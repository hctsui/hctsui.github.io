document.addEventListener("DOMContentLoaded", () => {
  /* Mobile-safe contact form and Cloudflare Turnstile. */
  const contactStyle = document.createElement("style");
  contactStyle.id = "contact-mobile-safety";
  contactStyle.textContent = `
    ...
    .turnstile-responsive-frame {
      width: 100%; max-width: 100%; min-width: 0; overflow: hidden;
    }
    .turnstile-responsive-frame > .cf-turnstile { transform-origin: top left; }
    @media (max-width: 420px) {
      .contact-page-section .container {
        width: min(calc(100% - 20px), var(--max));
      }
      body[data-page="contact"] .contact-form-shell { padding: 18px 14px; }
    }
  `;
  document.head.append(contactStyle);

  const installResponsiveTurnstile = () => {
    document.querySelectorAll(".cf-turnstile").forEach((widget) => {
      let frame = widget.closest(".turnstile-responsive-frame");
      if (!frame) {
        frame = document.createElement("div");
        frame.className = "turnstile-responsive-frame";
        widget.before(frame);
        frame.append(widget);
      }
      const baseWidth = 300;
      const baseHeight = 65;
      const resize = () => {
        const available = Math.max(1, frame.clientWidth || baseWidth);
  ...
  document
    .querySelectorAll(`[data-nav="${CSS.escape(page || "")}"]`)
    .forEach((active) => {
  ...
  document.querySelectorAll("[data-site-search]").forEach((shell) => {
    ...
    input.addEventListener("input", async () => {
      ...
      try {
        const lang =
          input.dataset.searchLanguage || document.documentElement.lang || "en";
        const items = (await loadIndex(input.dataset.searchIndex)).filter(
        ...
        if (!matched.length) {
          results.innerHTML = `<div class="site-search-empty">${
            lang.startsWith("zh") ? "找不到結果" : "No results"
          }</div>`;
        } else {
          results.innerHTML = matched
            .map(
              ({ item }) =>
                `<a href="${escapeHtml(item.url)}"><strong>${escapeHtml(
                  item.title,
                )}</strong>${
                  item.description
...
document.addEventListener("click", async (event) => {
  const toggle = event.target.closest(
    "[data-citation-toggle], [data-bibtex-toggle]",
  );
  if (toggle) {
    const panelId =
      toggle.dataset.citationToggle || toggle.dataset.bibtexToggle || "";
    const panel = document.getElementById(panelId);
    if (!panel) return;
    const opening = panel.hidden;
    document
      .querySelectorAll(".citation-panel:not([hidden])")
      .forEach((other) => {
        if (other === panel) return;
        other.hidden = true;
        document
          .querySelector(
            `[data-citation-toggle="${CSS.escape(
              other.id,
            )}"], [data-bibtex-toggle="${CSS.escape(other.id)}"]`,
          )
    ...
    if (opening) {
      panel
        .querySelector(".citation-format-tab.active")
  ...
  if (closeButton) {
    ...
    const trigger = document.querySelector(
      `[data-citation-toggle="${CSS.escape(
        panelId,
      )}"], [data-bibtex-toggle="${CSS.escape(panelId)}"]`,
    );
  ...
  if (formatButton) {
    const panel = document.getElementById(
      formatButton.dataset.citationPanel || "",
    );
    if (!panel) return;
  ...
  const copyButton = event.target.closest(
    "[data-copy-citation], [data-copy-bibtex]",
  );
  if (!copyButton) return;
  const targetId =
    copyButton.dataset.copyCitation || copyButton.dataset.copyBibtex || "";
...
document.addEventListener("submit", async (event) => {
  ...
  if (submit) {
    ...
    submit.textContent = document.documentElement.lang.startsWith("zh")
      ? "傳送中…"
      : "Sending…";
  }
  if (status) {
    ...
    if (status) {
      status.textContent =
        ...
        (document.documentElement.lang.startsWith("zh")
          ? "訊息已送出。"
          : "Message sent.");
      status.className = "contact-form-status success-message";
    }
  } catch (error) {
    if (status) {
      status.textContent = document.documentElement.lang.startsWith("zh")
        ? `傳送失敗：${error.message || error}`
        : `Could not send: ${error.message || error}`;
      status.className = "contact-form-status error-message";
    }
  } finally {
    if (submit) {
